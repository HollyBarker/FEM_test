...
int main()
{
	...
	readFile(U_initial_guess,"U_vector4.txt");
	readFile(V_initial_guess,"V_vector4.txt");
	...
	double dV_ds_point=0.0, dV_dX_point=0.0, V_point=0.0, U_point=0.0;
	...
	for (int e=1;e<=no_elements;e++) //loop over the elements
	{
		...
		for (int i=0; i<gauss_position.size(); i++) //Loop over the gauss points within the element
		{
			...
			U_point+=U[glob_node_no-1]*phi1(gauss_position[i]); //This is U_{(1,e)}*phi_1
			...
			U_point+=U[glob_node_no-1]*phi2(gauss_position[i]); // This is U_{(2,e)}*phi_2
			...
			for (int j=1; j<=no_nodes_per_element;j++) //Loop over local nodes
			{
				...
				if (eq_no!=-1) //If equaton no not equal to -1
				{
					...
					if (j==1) 
					{
						...
						element_residual_vec_V[i_loc_eq_no-1]+=(dV_dX_point*dphi1_dx_point+f_X_point*phi1_point+U_point*phi1_point)*Jacobianxs*gauss_weight[i];
					}
					if (j==2)
					{
						element_residual_vec_U[i_loc_eq_no-1]+=(dU_dX_point*dphi2_dx_point+V_point*phi2_point)*Jacobianxs*gauss_weight[i];
						element_residual_vec_V[i_loc_eq_no-1]+=(dV_dX_point*dphi2_dx_point+f_X_point*phi2_point+U_point*phi2_point)*Jacobianxs*gauss_weight[i];
