...
int main()
{
	...
	double phi1_point=0.0, phi2_point=0.0, dphi1_dx_point=0.0, dphi2_dx_point=0.0, MULTIPLIER, CROSSMULTIPLIER, CROSSMULTIPLIER2, u_point, v_point;
	count=0;
	...
	for (int indep_var_no=1; indep_var_no<=2; indep_var_no++)
	{
		for (int e=1;e<=no_elements;e++) //loop over the elements
		{
			...
			for (int i=0; i<gauss_position.size(); i++) //Loop over the gauss points within the element
			{
				...
				u_point+=U[glob_node_no-1]*phi1_point; // u_1 phi_1 at this node
				u_point+=U[glob_node_no-1]*phi2_point; // u_2 phi_2 at this node
				...
				for (int j=1; j<=no_nodes_per_element;j++) //Loop over local nodes
				{
					...
					if (eq_no!=-1) //If equaton no not equal to -1
					{
						...
						{
							element_residual_vec[i_loc_eq_no-1]+=(dU_dX_point*dphi1_dx_point+f_X_point*phi1_point+u_point*phi1_point)*Jacobianxs*gauss_weight[i];
						}
						else if (j==2 && indep_var_no==2)
						{
							element_residual_vec[i_loc_eq_no-1]+=(dU_dX_point*dphi2_dx_point+f_X_point*phi2_point+u_point*phi2_point)*Jacobianxs*gauss_weight[i];
						}
						for (int k=1;k<=no_nodes_per_element;k++) //Loop over local nodes
						{
							...
							if (eq_no!=-1) // If eq no not -1
							{
								...
								if(indep_var_no==2)cross_element_Jacobian_matrix(i_loc_eq_no-1,j_loc_eq_no-1)+=CROSSMULTIPLIER*Jacobianxs*gauss_weight[i]; //This is dRv/du. I know 
																								//theyre the same.
							}
						}
				...
				{	
					...
					for (int j_dof=1; j_dof<=N_dof;j_dof++)
					{
						...
						if(indep_var_no==2) glob_Jacobian_matrix(i_glob_eq_no-1,j_glob_eq_no-99-1)+=cross_element_Jacobian_matrix(i_dof-1,j_dof-1);
					}
				}
